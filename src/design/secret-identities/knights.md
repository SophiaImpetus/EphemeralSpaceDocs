...
Their time travel abilities only let them jump back around 100 years at a time, and each jump they're less likely to be successful again, leaving them an infrequent, but notable and terrifying occurrence in the current era.
They seek to overcome this by repeatedly tearing at the fabric of reality by creating anomalies and paradoxes in the time they're in, and using them to leap further back, over and over.

The key to their plan is killing their own ancestors, preventing themselves from ever being born (or at least altering their future in some way to create a paradox of some kind).
Once they manage this, and have caused enough instability to fray the timeline, they can utilize the most powerful power source on the station, the AME, to gain the energy needed to travel further back in time.
...
Knights-Aberrant are unwelcome in the time they have traveled into. Only by feeding on Distortion created by temporal damage can they remain in their position long enough to avoid being flung back to whence they came from, near the end of the universe.
At the start of the round, 20 mins later, and then every 15 mins thereafter, they are given a new goal to complete as a team to harvest the distortion needed to survive.
Failing to complete their goal before the next arrives instantly banishes away one of them at random - when only 3 minutes remain, the one who would be sent back to the future is marked to their team.

These "Survival Goals" Can be one of the following.
- People on board the ship have taken at least X damage over the duration.
- Create an anomaly at Location. To create an anomaly, one must be in Aberrant form and go to the targeted area, then use the Create Anomaly ability.
- At least 1 person has died over the duration.

These goals are fairly open for a good reason - Crew should be aware that reckless playing allows the Knights to avoid needing to directly influence the round, and gives them stealth for longer to peruse their other goals, prepare advantages and outmatch the crew.
Likewise, playing cautiously as crew may force the Knights hand and put one of them in a position where crew can fight back if they fight for their own life (or force one to be banished away).

# Infinite Escapism

In order to escape, survival is not enough.

Each Knight has two individual goals.
- Each Knight has an ancestor on board the station, and that ancestor must die in order for them to be able to proceed further into the past.
...
They can then move onto the last phase.
...
Time travel requires intense psionic concentration and destroys all mundane objects, preventing the Knights from bringing any equipment with them.
This isn't an issue to them, given they can psionically manifest the powers needed for combat after absorbing enough energy.
However, it makes them hard to spot when they can suddenly transform, or that they're any more dangerous than your average crew.

Fortunately for crew, all Knights have an aspect which may tip crew off to their nature. One of the following is randomly selected for each Knight generated in a round.
- Named after an ancestor/Ancestor's family name: The Knight has the same first or last name as their target.
- Looks like their target: The Knight looks very similar to their target, but has a different haircut and may be any gender.
- Ancestral kinship: The Knight has the same likes, dislikes and interests as their target.
- Psionic Malevolence: The Knight gives the Hunted modifier to their target.

Importantly, the Knight does NOT know which of these is active (though they can probably guess if it is the name one, it is still possible to have matching names via typical generation).
If this is used to induce paranoia in crew who have superficial similarities to each other, or successfully avoid dangerous Knights who are out to kill you, is expected to vary every round.

# Knight Anomalies

Knight's anomalies don't have any special equipment like APEs or Scanners in order to close them. Instead they're logically fixed by using supplies anyone on the station has access to.
A fire anomaly can be sealed by pouring enough water onto it, or intensified with fuel, for instance.
Knights in their Aberrant form are also immune to being targeted by their own anomalies, allowing them to utilize them to their advantage.
This should be reflected in any anomaly created for a given Crest.
...
Knights swap from stealth to loud at the flip of a coin, but most Knights will not want to be seen making this change. The ideal strategy is get in, get out, don't be seen - especially for placing anomalies.
